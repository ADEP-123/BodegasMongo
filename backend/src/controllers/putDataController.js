        ...
        if (result.length != 0) {
            ...
            if (cantidadExistenteB1 > cantidad) {
                ...
                if (result.length != 0) {
                    ...
                    result = await postInventarioService(123456789, bodega2, producto, cantidad, usuario)
                    res.status(200).json({ message: "Se ha actualizado 1 inventario y creado un inventario nuevo" })
                }
            } else {
